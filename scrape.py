...
MAX_RETRIES = 3
BASE_BACKOFF = 3.0
PAGE_DELAY = 0.5  # politeness delay between pages of the same store
...
async def process_store(client: httpx.AsyncClient, store: dict, state: dict, sem: asyncio.Semaphore) -> list[dict]:
    domain = store["domain"]
    async with sem:
        await asyncio.sleep(0.3)  # jitter so requests don't fire in lockstep
        try:
            products = await fetch_all_products(client, store["url"])
